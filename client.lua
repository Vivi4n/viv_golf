	...
	while true do
		...
		if distance < 1.5 then 
			...
			if golfplaying then
				...
				DisplayHelpText("Press ~g~E~s~ to start golf.") 
			end
...
function spawnCart()
    ...
    local plate = GetVehicleNumberPlateText(spawned_car)
    local vehNet = NetworkGetNetworkIdFromEntity(spawned_car)
...
function endShot()
	...
	if distance < 1.5 then
		exports['mythic_notify']:SendAlert('success', 'You got the ball with in range!', 15000)
		totalgolfstrokes = golfstrokes + totalgolfstrokes		
		golfstrokes = 0
		ballstate = 1
	end
	if golfstrokes > 12 then
		exports['mythic_notify']:SendAlert('error', 'You took too many shots..', 15000)
		totalgolfstrokes = golfstrokes + totalgolfstrokes		
		golfstrokes = 0
		ballstate = 1
	end
